...
| MP-WRAP | hard_gate | 32-bit end-address arithmetic wraparound | Near-`32'hffff_ffff` directed cases | Address error before any DMA data request | planned | No formal proof |
| MP-MODES | hard_gate | Fixed, incrementing, chunk-wrap, and non-wrapped multi-chunk footprints | Mode/width/direction matrix in directed sequence | RTL and model agree with expected result | planned | Only configured cases imply coverage |
| MP-WIDTHS | hard_gate | One-, two-, and four-byte transfer widths including final partial access | Cross each supported width with boundary cases | Correct inclusive final-byte decision | planned | No unsupported width claim |
