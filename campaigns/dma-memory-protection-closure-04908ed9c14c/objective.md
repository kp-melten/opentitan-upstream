...
- The DMA rejects an OT-memory import or export before any source-read or
  destination-write data request when the physical accessed-byte footprint is
  below the inclusive base, above the inclusive limit, or wraps a 32-bit
  address space.
...
- A directed `dma_mem_boundary_vseq`, inclusive-range model and scoreboard
  agreement, invalid-configuration no-data-request checking, testplan/sim
...
| MP-RANGE | hard_gate | Any physical byte below base or above limit raises direction-correct address error | behavior | baseline semantics disagree across RTL/model/scoreboard | directed source/destination cases | UVM/XSim/static evidence | human_controller | active | interrupt-clear exception pending controller answer |
...
| MP-NO-DATA-REQ | hard_gate | Invalid configuration emits no source-read or destination-write data request | transaction | baseline validates in `DmaAddrSetup` | scoreboard fatal checks and directed invalid cases | UVM evidence when available | verification_owner | active | configured interrupt-clear writes are provisionally excluded |
